...
/* ------------------------------------------------------------------
   Sistema de ornamentos de la plantilla "Boda de Jorge & Macarena".
   Todo el arte es SVG inline de línea fina inspirado en la flora de la
   Ciudad de México: flor de granada (naranja/rosa), bugambilia (rosa),
   cempasúchil (amarillo/naranja) y hojas verdes finas. Los tallos y
   hojas se colorean con `currentColor` (el consumidor decide el tinte
   vía clases de color con variables `--inv-*`); las flores usan sus
   propios acentos (`--inv-accent-orange`, `--inv-accent-pink`,
   `--inv-accent-yellow`) vía `style`. Todo `aria-hidden`.
------------------------------------------------------------------ */
...
/* Flor de granada: 5 pétalos naranjas y corazón rosa. */
function GranadaFlower({ x = 0, y = 0, s = 1 }) {
  return (
    <g transform={`translate(${x} ${y}) scale(${s})`}>
      {[0, 72, 144, 216, 288].map((a) => (
        <ellipse
          ...
          ry="7.5"
          transform={`rotate(${a})`}
          style={{ fill: "var(--inv-accent-orange)" }}
          opacity="0.9"
        />
      ))}
      <circle r="3" style={{ fill: "var(--inv-accent-pink)" }} />
    </g>
  );
}

/* Bugambilia: tres brácteas rosas con centro amarillo. */
function Bougainvillea({ x = 0, y = 0, s = 1 }) {
  return (
    <g transform={`translate(${x} ${y}) scale(${s})`}>
      {[0, 120, 240].map((a) => (
        <path
...
/* Cempasúchil: pom amarillo con centro naranja. */
function Marigold({ x = 0, y = 0, s = 1 }) {
  return (
    <g transform={`translate(${x} ${y}) scale(${s})`}>
      {[0, 60, 120, 180, 240, 300].map((a) => (
        <ellipse
          ...
          ry="4.5"
          transform={`rotate(${a})`}
          style={{ fill: "var(--inv-accent-yellow)" }}
          opacity="0.9"
        />
      ))}
      <circle r="2" style={{ fill: "var(--inv-accent-orange)" }} />
    </g>
  );
}
...
/* ------------------------------------------------------------------
   ...
   flora (granada + bugambilia + cempasúchil). Se usa en las 4 esquinas
   de la portada (con rotaciones/espejos) y en tarjetas.
------------------------------------------------------------------ */
export function BotanicalCorner({ className = "" }) {
  return (
    ...
    >
      ...
      {/* Flora mexicana */}
      <GranadaFlower x={116} y={44} s={1.1} />
      <Bougainvillea x={62} y={96} s={1} />
      <Marigold x={34} y={118} s={0.85} />
    </svg>
  );
}
...
/* ------------------------------------------------------------------
   ...
   simétrica con hojas y una flor de granada al centro flanqueada por
   dos cempasúchiles.
------------------------------------------------------------------ */
export function BotanicalDivider({ className = "" }) {
  return (
    ...
    >
      ...
      <Marigold x={294} y={36} s={0.7} />
      <Marigold x={346} y={36} s={0.7} />
      {/* Flor central de granada (5 pétalos) */}
      <GranadaFlower x={320} y={18} s={1.15} />
    </svg>
  );
}

/* ------------------------------------------------------------------
   Floritura de párrafo: filetes cortos, hoja, bugambilia central y
   bayas.
------------------------------------------------------------------ */
export function Flourish({ className = "" }) {
  return (
    ...
    >
      ...
      {/* Bugambilia central */}
      <Bougainvillea x={90} y={14} s={0.9} />
    </svg>
  );
}

/* ------------------------------------------------------------------
   Título de sección propio: eyebrow con filetes, título en serif
   Playfair verde (o script rosa con `script`) y separador botánico.
   Paleta clara: eyebrow rosa, título verde, orla botánica verde.
------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------
   ...
   visualmente un marco botánico fino): doble borde verde/amarillo con
   esquinas recortadas y biseles diagonales. Decorativo (aria-hidden) y
   sin eventos de puntero; el contenido vive en la capa `relative`.
------------------------------------------------------------------ */
