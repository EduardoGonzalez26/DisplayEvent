...
/* ------------------------------------------------------------------
   Itinerario minimalista (XV de Alice): timeline vertical con línea fina
   dorada (izquierda en móvil, centro en desktop), nodos de anillo dorado
   fino con número, tarjetas SIN fondo con borde dorado (móvil) o
   inferior dorado (desktop), hora en chip rosa/dorado y título serif.
   En desktop los momentos alternan lados; stagger reveal. Null si no hay
   items (misma paridad que shared/Itinerary).
------------------------------------------------------------------ */
          ...
          >
            {items.map((it, i) => {
              ...
              return (
                ...
                >
                  {/* Nodo: anillo dorado fino con número */}
                  ...
                  {/* Contenido: alterna lados en desktop, centrado en móvil.
                      Borde izquierdo dorado en móvil, inferior en desktop. */}
                  <div
                  ...
                  >
                    {it.time && (
                      <span className="inline-block rounded-full border border-[var(--inv-accent-yellow)]/50 bg-[var(--inv-surface)] px-3 py-1 text-[0.7rem] uppercase tracking-[0.3em] tabular-nums text-[var(--inv-accent-pink)] md:text-xs">
                        {it.time}
                      </span>
                    )}
