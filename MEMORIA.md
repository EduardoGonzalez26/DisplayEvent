...
- Fuentes **Cormorant Garamond** + **Poppins** + **Dancing Script** + **Lato**. El **monograma de la portada** usa además **Great Vibes** vía la clase global `.font-script` de `index.css` (cargada en `client/index.html`), independiente del `--inv-font-script` del tema (Dancing Script).
...
> **Iteración del 2026-09-16 (en el commit de esta fecha):** cabecera de la Mesa de Regalos simplificada —el eyebrow "Regalos" pasa a encabezado con `largeEyebrow` (`title` ahora opcional en `WeddingSectionTitle`); se retiran el título grande, el uso de `.text-gold-gradient-deep` y la label `registryTitle`— y **monograma de la portada en Great Vibes** (clase global `.font-script`). Iteraciones previas del mismo día, ya commiteadas: retiro del bloque de transferencia, intro dinámica y textos fijos (`a38d940`; memoria en `bdb245f`), después de Padres/Padrinos + Mesa de Regalos informativa + bloqueo de scroll del sobre (`a241198`, §7; memoria en `61520ef`).
